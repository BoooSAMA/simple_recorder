...
> **多平台直播音频录制工具** — 基于 Flutter 构建，支持 Bilibili / 斗鱼 / 虎牙 / 猫耳FM 多平台直播间音频录制。
>
> **录播仅限自用，禁止将录播文件分发至网上。**
...
2. 系统会批量检测所有收藏主播的直播状态（B站主播合并为一次请求，其余平台并发查询）：
   - 🟢 **直播中** — 绿色高亮，显示在列表上方
   - ⚫ **未开播** — 灰色，收在下方
...
- **多平台支持** — Bilibili、斗鱼、虎牙、猫耳FM
- **纯音频录制** — 基于 FFmpeg 仅录制音频流 (c:a copy, 不重编码)，节省存储空间
- **并行录制** — 最多同时录制 10 个直播间，互不干扰
...
- **B站批量检测** — 收藏的 B站主播合并为一次批量请求（每批最多 50 个房间），N 个请求缩减为 1-2 个
- **并发池检测** — 其余平台主播按并发上限并行查询，避免阻塞 UI
...
- **B站批量状态接口** — 单个主播逐一请求合并为按房间号批量查询（`getRoomBaseInfo`），收藏 N 个 B站主播仅需 ⌈N/50⌉ 次请求，批量失败自动降级逐个
- **直播状态缓存** — 上次检测结果持久化到本地（`last_live_status`），冷启动立即恢复显示，后台再刷新真实状态；检测失败不覆盖旧缓存
...
- [x] 快速启动、非阻塞权限请求
- [x] B站批量状态检测（一次请求查最多 50 个房间，失败自动降级）
- [x] 直播状态本地缓存（冷启动秒显上次状态）
